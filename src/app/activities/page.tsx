...
const typeConfig: Record<string, { label: string; color: string; bgClass: string }> = {
  ...
  sleepover: { label: 'SLEEPOVER', color: '#1B3A5C', bgClass: 'bg-blue-50 text-blue-700' },
      ...
      <div className="p-7">
        <div className="flex justify-between items-center mb-6">
          <div className="flex">
            {['Todas', 'Machanot', 'Viajes', 'Sleepovers'].map((t, i) => (
              <button key={t} className={`px-5 py-2 text-sm font-medium border transition-all ${i === 0 ? 'bg-[#0F2440] text-white border-[#0F2440]' : 'bg-white text-[#5A6472] border-[#E2DFD8]'} ${i === 0 ? 'rounded-l-lg' : ''} ${i === 3 ? 'rounded-r-lg' : ''} ${i > 0 ? 'border-l-0' : ''}`}>
                {t}
              </button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {mockActivities.map((act) => {
            ...
            return (
              <div key={act.id} className="bg-white rounded-xl border border-[#E2DFD8] p-5 shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all cursor-pointer">
                <div className="text-[0.68rem] uppercase tracking-[0.1em] font-bold mb-2" style={{ color: cfg.color }}>{cfg.label}</div>
                <h3 className="text-base font-semibold text-[#0F2440] mb-1.5">{act.name}</h3>
