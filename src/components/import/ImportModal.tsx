  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      if (data.members.length === 0) {
        throw new Error('No members found in the file.');
      }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#D8E1EA]">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-serif font-bold text-lg text-[#1B2A6B]">Import SOM Attendance</h2>
          </div>
        ...
        <div className="px-6 py-5">
          {loading ? (
            <div className="text-center py-10">
              ...
              <p className="text-sm text-[#5A6472]">Processing attendance file...</p>
            </div>
          ) : result ? (
            /* Success */
            <div>
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  <h3 className="font-semibold text-[#1A1A2E]">Attendance imported successfully</h3>
                  <p className="text-sm text-[#5A6472]">{result.members.length} members · {totalSessions} sessions</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                <div className="bg-green-50 rounded-lg p-3 text-center">
                  ...
                  <div className="text-[0.68rem] text-[#5A6472] font-medium">Attendance</div>
                </div>
                <div className="bg-blue-50 rounded-lg p-3 text-center">
                  <div className="text-xl font-bold text-[#1B2A6B]">{totalPresent}</div>
                  <div className="text-[0.68rem] text-[#5A6472] font-medium">Present</div>
                </div>
                <div className="bg-red-50 rounded-lg p-3 text-center">
                  <div className="text-xl font-bold text-[#C0392B]">{totalAbsent}</div>
                  <div className="text-[0.68rem] text-[#5A6472] font-medium">Absent</div>
                </div>
              </div>

              {/* Monthly breakdown */}
              <div className="bg-[#FAFAF8] rounded-lg p-3 mb-4">
                <p className="text-xs font-semibold text-[#5A6472] uppercase tracking-wider mb-2">Sessions by month</p>
                {result.months.map(m => (
                  <div key={m.name} className="flex justify-between text-sm py-1">
                    <span className="text-[#1A1A2E]">{m.name}</span>
                    <span className="font-medium text-[#1B2A6B]">{m.dates.length} sessions</span>
                  </div>
                ))}
              </div>
            ...
            <div>
              {isImported && (
                <div className="flex items-center justify-between bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 mb-4">
                  <p className="text-sm text-amber-800">There is already imported data. Uploading a new file will replace it.</p>
                  <button
              ...
              >
                ...
                <p className="text-sm font-medium text-[#1A1A2E] mb-1">
                  Drag the SOM attendance file here
                </p>
                <p className="text-xs text-[#5A6472]">
                  Format: SOM ATTENDANCE.xlsx
                </p>
                <input
                  ref={inputRef}
                  type="file"
              ...
              <p className="text-xs text-[#5A6472] mt-3 text-center">
                The file must have: FirstName, LastName, Contact ID, and date columns with true/false
              </p>
            </div>
          )}
        </div>
