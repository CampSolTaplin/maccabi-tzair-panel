...
export default function Sidebar() {
  ...
  return (
    <nav
      className={cn(
        'fixed left-0 top-0 bottom-0 z-50 flex flex-col bg-[#0F2440] transition-all duration-300 ease-in-out',
    ...
    >
      ...
      <div className="flex items-center gap-3.5 border-b border-white/[0.08] px-5 py-5 min-h-[80px]">
        <div className="w-9 h-9 flex-shrink-0 flex items-center justify-center">
          <Star className="w-8 h-8 text-[#D4A843] fill-[#D4A843]" />
        </div>
        <div
          className={cn(
        ...
        >
          <h1 className="font-serif text-[1.1rem] font-bold text-[#D4A843] leading-tight tracking-wide">
            Maccabi Tzair
          </h1>
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive
                  ? 'text-[#E8C96A] bg-[#D4A843]/[0.12]'
                  : 'text-white/55 hover:text-white/85 hover:bg-white/[0.06]'
              )}
            >
              {isActive && (
                <span className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 bg-[#D4A843] rounded-r" />
              )}
